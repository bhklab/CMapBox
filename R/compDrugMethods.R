...
compDrugMethods = function(numAnalyses, geneIds, geneEsts, drugPert, pharmSet, pvals, drugScoreMeth, pCut = TRUE, cutOff = 0.05, genesToStart = 0.20, numbIters = 10, gwcMethod = "spearman", numbPerms = 1000, volcPlotEsts = NULL, drugEst = TRUE, extraData = NA, extraCut = NA, extraDirec = NA, drugNameVec = NULL)
{
  ...
  paramListsOrig = list(numAnalyses, geneIds, geneEsts, drugPert, pharmSet, pvals, drugScoreMeth, pCut, cutOff, genesToStart, numbIters, gwcMethod, numbPerms, volcPlotEsts, drugEst, extraData, extraCut, extraDirec)
  numbArgs = length(paramListsOrig)
  paramList = list()
  tempList = list()
  #shuld probably replace below with a check to make sure all the lists are the same length
  #as user has made some sort of error if they are not
  for(i in 1:numbArgs)
  {
  ...
  for(i in 1:numAnalyses)
  {
    ...
    drugResList[[i]] = rankDrugsGwc(paramList[[2]][[i]], paramList[[3]][[i]], paramList[[4]][[i]], paramList[[5]][[i]], paramList[[6]][[i]],
                                    paramList[[7]][[i]], paramList[[8]][[i]], paramList[[9]][[i]], paramList[[10]][[i]], paramList[[11]][[i]],
                                    paramList[[12]][[i]], paramList[[13]][[i]], paramList[[14]][[i]], paramList[[15]][[i]],
                                    inspectResults = FALSE, showMimic = FALSE, mDataType = "rna", paramList[[16]][[i]],
                                    paramList[[17]][[i]], paramList[[18]][[i]])
