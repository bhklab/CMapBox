...
#' geneDataClean = cleanData(geneIds = geneDataGwc$symbol, geneEsts = geneDataGwc$logFC, pvals = geneDataGwc$P.Value)
#' ##keep genes with larger abs(logFC) of genes with the same id. Could also supply entrez ids
#' geneDataClean = cleanData(geneIds = geneDataGwc$ensembl_id, geneEsts = geneDataGwc$logFC, pvals = geneDataGwc$P.Value, byP = FALSE)


cleanData = function(geneIds, geneEsts, pvals, byP = TRUE, forRankAndPlot = FALSE)
{
  if(is.element("org.Hs.eg.db", installed.packages()[,1]) == FALSE)
  {
